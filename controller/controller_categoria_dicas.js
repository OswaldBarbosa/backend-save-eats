/***************************************************************************************************************************************************
 * Objetivo: Responsavel pela regra de negocio referente ao CRUD da CATEGORIA das dicas
 * Data: 07/09/2023
 * Autor: Julia Soares
 * Versão: 1.0
 ***************************************************************************************************************************************************/
...
var categoriaDicasDAO = require('../model/DAO/categoria_dicasDAO.js')

const { request } = require('express')

const inserirCategoriaDicas = async function (dadosCategoria) {
    ...
    if (dadosCategoria.categoria == '' ||dadosCategoria.categoria == undefined || dadosCategoria.categoria.length > 45
        ...
        let resultDados = await categoriaDicasDAO.insertCategoriaDicas(dadosCategoria)

        //Valida se o banco de dados inseriu corretamente os dados
        if (resultDados) {

            let novaCategoria = await categoriaDicasDAO.selectLastId()
...
const deletarCategoriaDicas = async function (idCategoria) {
    let statusId = await categoriaDicasDAO.selectCategoriaDicasByID(idCategoria);

    if (statusId) {
        if (idCategoria == '' || idCategoria == undefined || isNaN(idCategoria)) {
            return message.ERROR_INVALID_ID; //Status code 400
        } else {
            let resultDados = await categoriaDicasDAO.deleteCategoriaDicas(idCategoria)
...
const atualizarCategoriaDicas = async function (dadosCategoria, idCategoria) {
    ...
    if (dadosCategoria.categoria == '' ||dadosCategoria.categoria == undefined || dadosCategoria.categoria.length > 45
        ...
        let statusId = await categoriaDicasDAO.selectLastId();

        if (statusId) {
            //Encaminha os dados para a model do cliente
            let resultDados = await categoriaDicasDAO.updateCategoriaDicas(dadosCategoria);
...
const getCategoriaDicas = async function () {
    let dadosCategoriasJSON = {};


    let dadosCategoria = await categoriaDicasDAO.selectAllCategoriaDicas();
...
const getCategoriaDicasPorID = async function (id) {
    ...
    if (id == '' || id == undefined || isNaN(id)) {
        ...
        let dados = await categoriaDicasDAO.selectCategoriaDicasByID(id)
...
module.exports = {
    inserirCategoriaDicas,
    deletarCategoriaDicas,
    atualizarCategoriaDicas,
    getCategoriaDicas,
    getCategoriaDicasPorID
}